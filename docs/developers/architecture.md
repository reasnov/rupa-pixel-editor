...
3.  **Service Layer (`src/lib/engine/services/`)**: The core business logic. Services manipulate state and orchestrate complex operations (e.g., Project management, Drawing algorithms).
4.  **Engine Layer (`src/lib/engine/`)**: Low-level infrastructure and orchestrators (Input parsing, Audio synthesis, Export algorithms).
5.  **Logic Layer (`src/lib/logic/`)**: The "Brain" of the editor. Pure, stateless algorithms and mathematical models (Geometry, Path processing, Color theory).
...
- **Input Engines**:
  - **KeyboardEngine**: Keyboard-centric semantic action mapping.
  - **PointerEngine**: Mouse-based positional intent and "Continuous Flow" mapping.
...
3.  **Editor Engine**: TypeScript modules handling non-UI concerns:
    ...
    - `shuttle.ts`: High-level service coordinator.
    - `mode.svelte.ts`: Behavioral mode orchestration.
...
The central heart of the application. It manages the editor environment through specialized sub-state classes:
...
- **Positional Awareness**: Maps screen-space coordinates to the Canvas grid-space.
...
The internationalization layer for the editor.
...
The "Brain" of the editor. This layer contains pure, side-effect-free algorithms.

- **Geometry Module**: Handles line interpolation (Bresenham) and arc fitting for shape correction.
- **Path Module**: Manages smoothing and simplification (Douglas-Peucker) of barista strokes.
...
- **Inspector Sidebar (Right)**: Property panels for Color selection, history, and Cursor statistics.
- **Status Footer (Bottom)**: Houses the **TechLedger** with modular indicators for Rhythms, Modes, Working Time, and Ambiance status.

### 5.2 The Camera Protocol

Viewport movement is achieved by applying CSS transforms to the **Canvas**. The system ensures that zooming focal points are always centered on the current `cursorPos`, maintaining the barista's focus.
...
To maintain visual harmony and code maintainability, Rupa follows the **SMACSS** (**S**calable and **M**odular **A**rchitecture for **CSS**) approach, adapted for Svelte 5 and Tailwind CSS:

1.  **Base**: Default styles for HTML elements. In Rupa, these are managed via Tailwind's `@base` layer and global `layout.css`.
2.  **Layout**: Major structural components (e.g., Sidebars, Header, Timeline Panel). These define the skeleton of the Editor.
...
5.  **Theme**: Visual skinning (e.g., Solarized Cream, Barista Magenta). These are managed via CSS variables and Tailwind configuration.
...
2. Prefix shared editor classes with `editor-` (e.g., `.editor-panel`) to avoid naming collisions.
...
To ensure the editor is easily maintainable and scalable, all static data (Shortcuts, Palettes, Audio frequencies, Onboarding text) is stored in the **Data Layer** using JSON files in `src/lib/config/`.
