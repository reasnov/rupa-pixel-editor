...
*   **Optional Mouse Support:** Mouse interactions (click, drag, scroll) are treated as supplemental overlays to the core keyboard engine. They must stay synchronized with the `cursorPos` but should never be the *exclusive* way to access a feature.
