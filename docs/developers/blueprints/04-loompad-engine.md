...
3.  **Navigation Sovereignty:** Navigation keys (Arrows) are treated as "Elastic Chords" that allow modifiers to pass through without changing the base Intent, instead modifying the *Mode* of that Intent (e.g., Move vs. Move-and-Stitch).
...
- *Behavior:* Modifies the `AtelierState` flow flags on `keydown` and reverts them on `keyup`.
- *Example:* `Flow-Stitch`, `Flow-Select`.
