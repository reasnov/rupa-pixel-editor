...
export class ExportEngine {
     ...
     */
    static toSVG(width, height, data, bgColor = 'transparent', includeBorders = false) {
        ...
        if (includeBorders) {
            ...
            const clustersByColor = new Map();
            for (let i = 0; i < data.length; i++) {
                const val = data[i];
                if (val === 0)
                    continue;
                const color = ColorLogic.uint32ToHex(val);
                if (!clustersByColor.has(color))
                    clustersByColor.set(color, new Set());
     ...
     */
    static async toRaster(width, height, data, scale, format = 'png', bgColor = 'transparent', includeBorders = false) {
     ...
     */
    static toAnimatedSVG(width, height, framesData, frameDurations, bgColor = 'transparent', includeBorders = false) {
     ...
     */
    static async toVideo(width, height, framesData, frameDurations, scale, format = 'webm', bgColor = 'transparent', includeBorders = false) {
        const canvas = document.createElement('canvas');
     ...
     */
    static async toGIF(width, height, framesData, frameDurations, scale, bgColor = 'transparent', includeBorders = false) {
     ...
     */
    static quantizeFrame(data) {
        const paletteMap = new Map();
        const palette = [];
        const indexedPixels = new Array(data.length / 4);
