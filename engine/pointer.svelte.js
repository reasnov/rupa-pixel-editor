...
export class PointerEngine {
    ...
    mapToGrid(clientX, clientY, canvasElement) {
        const rect = canvasElement.getBoundingClientRect();
        return Geometry.mapScreenToGrid(clientX, clientY, rect, state.canvas.width, state.canvas.height);
    }
    handleStart(e, canvasElement) {
        ...
        const isShading = state.studio.isShadingLighten || state.studio.isShadingDarken || state.studio.isShadingDither;
    ...
    handleMove(e, canvasElement) {
        ...
        this.rafId = requestAnimationFrame(() => {
            ...
            // --- Pan Mode Support ---
            if (currentMode === 'PAN') {
                const delta = Geometry.calculatePanDelta(e.clientX, e.clientY, this.lastScreenPos?.x || e.clientX, this.lastScreenPos?.y || e.clientY);
            ...
            const isShading = state.studio.isShadingLighten ||
