...
export class DrawService {
                ...
                for (let px = 0; px < pb.width; px++) {
                    ...
                    if (editor.canvas.isValidCoord(targetX, targetY)) {
                        ...
                        if (editor.selection.isActive && !editor.selection.activeIndicesSet.has(index))
                            continue;
                        if (editor.studio.isAlphaLocked &&
    ...
    getEffectColorForPoint(x, y, targetVal) {
        if (mode.current.type === 'ERASE')
            return 0;
    ...
    continueStroke(x, y, lastX, lastY) {
        ...
        if (arcData && arcData.isFull) {
            ...
            if (simplified.length === 2) {
                ...
                for (let i = 0; i < visualPoints.length - 1; i++) {
                    finalPixels.push(...Geometry.getLinePoints(visualPoints[i].x, visualPoints[i].y, visualPoints[i + 1].x, visualPoints[i + 1].y, stab / 100));
                }
            }
            else {
    ...
    commitShape() {
        ...
        if (tool === 'GRADIENT') {
            ...
            const gradientMap = PixelLogic.getLinearGradientMap(anchor.x, anchor.y, x, y, targetIndices, editor.canvas.width);
            const currentPixels = new Uint32Array(editor.canvas.pixels);
            const batch = [];
        ...
        if (tool === 'RECTANGLE') {
            ...
            points = PixelLogic.getPolygonPoints(anchor.x, anchor.y, x, y, editor.studio.polygonSides, editor.studio.polygonIndentation);
        }
        if (points.length > 0) {
