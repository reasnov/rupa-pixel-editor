...
export class PixelLogic {
     ...
     */
    static getSymmetryPoints(x, y, width, height, mode) {
        const points = [];
        const mx = width - 1 - x;
        const my = height - 1 - y;
        if (mode === 'HORIZONTAL') {
     ...
     */
    static floodFill(data, width, height, startX, startY, fillColor) {
    ...
    static scan(lx, rx, y, width, height, data, targetColor, stack) {
        if (y < 0 || y >= height)
            return;
        let added = false;
        for (let x = lx; x <= rx; x++) {
     ...
     */
    static pixelPerfectFilter(points) {
        if (points.length < 3)
            return points;
     ...
     */
    static getRectanglePoints(x1, y1, x2, y2) {
        const points = [];
     ...
     */
    static getEllipsePoints(x1, y1, x2, y2) {
        const xc = (x1 + x2) / 2;
        const yc = (y1 + y2) / 2;
    ...
    static addEllipsePoints(points, xc, yc, x, y) {
     ...
     */
    static getPolygonPoints(x1, y1, x2, y2, sides, indentation = 0) {
        const xc = (x1 + x2) / 2;
        const yc = (y1 + y2) / 2;
        const rx = Math.abs(x2 - x1) / 2;
        const ry = Math.abs(y2 - y1) / 2;
     ...
     */
    static getLinearGradientMap(x1, y1, x2, y2, targetIndices, width) {
        const results = [];
        const dx = x2 - x1;
        const dy = y2 - y1;
