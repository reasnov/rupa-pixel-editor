...
<div class="pointer-events-none absolute inset-0 z-20 overflow-hidden">
	...
	<div class="absolute top-0 bottom-0 left-1/2 w-px -translate-x-1/2 bg-text-main/40"></div>
	<!-- Horizontal Center (Rural Evergreen) -->
	<div class="absolute top-1/2 right-0 left-0 h-px -translate-y-1/2 bg-text-main/40"></div>
	...
	{#each [-24, -16, -8, 8, 16, 24] as offset}
		...
		{#if pos > 0 && pos < 100}
			<div
				class="absolute top-0 bottom-0 w-px -translate-x-1/2 bg-text-main/20"
				style="left: {pos}%"
			></div>
		{/if}
	{/each}
	{#each [-24, -16, -8, 8, 16, 24] as offset}
		...
		{#if pos > 0 && pos < 100}
			<div
				class="absolute right-0 left-0 h-px -translate-y-1/2 bg-text-main/20"
				style="top: {pos}%"
			></div>
		{/if}
	{/each}
