...
{#if editor.isCursorVisible}
	<div transition:fade={{ duration: 800 }} class="cursor-container absolute inset-0">
		...
		{#if studio.isAlphaLocked || studio.isColorLocked || studio.isShadingLighten || studio.isShadingDarken || studio.isShadingDither}
			<div
				transition:scale
				class="absolute -top-3 -right-3 flex items-center justify-center rounded-full border-2 border-canvas-bg bg-ui-accent p-1 text-[8px] font-bold text-white shadow-lg ring-1 ring-text-main/10"
			>
		...
		{#if editor.isPicking}
			<div
				...
				class="absolute inset-[-4px] z-0 animate-ping rounded-sm border-2 border-canvas-bg opacity-20"
			...
			>
				<div
					class="relative h-7 w-7 overflow-hidden rounded-xl rounded-br-none border-4 border-canvas-bg bg-canvas-bg shadow-xl ring-1 ring-text-main/10"
				...
				<div class="-mt-0.5 h-2 w-0.5 rounded-xl bg-canvas-bg opacity-80 shadow-sm"></div>
			</div>
		{/if}
	</div>
{/if}
