...
>
		...
		{#key mode.current.type}
			...
			>
				...
				{#if mode.current.type === 'READY'}
					<span
						class="ml-1 border-l border-text-main/20 pl-2 font-mono text-[9px] tracking-tighter not-italic"
					>
						v{editor.version}
					</span>
				{/if}
	...
	{#if editor.project.lastSaved}
		...
		<div
			class="text-[8px] font-bold tracking-[0.2em] text-text-main/20 uppercase"
			aria-label={__('workspace:project.saved', { replace: { time } })}
		>
			{__('workspace:project.saved', { replace: { time } })}
		</div>
	{/if}
</div>
