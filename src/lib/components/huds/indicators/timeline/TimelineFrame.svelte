...
>
	<div
		class="group relative h-full w-full cursor-pointer overflow-hidden rounded-md border border-text-main/10 transition-all {isActive
			? 'border-ui-accent bg-white shadow-lg ring-2 ring-ui-accent/20'
	...
	>
		...
		<!-- Top Label: Index -->
		<div
			class="absolute top-1 left-1 flex h-4 min-w-4 items-center justify-center rounded-sm bg-text-main/40 px-1 backdrop-blur-sm"
		>
		...
		<!-- Duration Badge -->
		<div
			class="absolute right-1 bottom-1 rounded bg-white/80 px-1.5 py-0.5 font-mono text-[9px] font-bold text-text-main/50 shadow-sm"
		>
			{frame.duration}ms
		</div>
	</div>
	...
	<div
		class="absolute top-0 right-0 bottom-0 z-30 w-2 cursor-col-resize rounded-r-md transition-colors hover:bg-ui-accent/40 active:bg-ui-accent"
		onmousedown={startResize}
	>
		<div
			class="absolute top-1/2 left-1/2 h-6 w-0.5 -translate-x-1/2 -translate-y-1/2 rounded-full bg-text-main/10 group-hover:bg-white/60"
		></div>
	</div>
</div>
