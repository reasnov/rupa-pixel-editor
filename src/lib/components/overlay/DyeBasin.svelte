<script lang="ts">
	...
	$effect(() => {
		...
		untrack(() => {
			h = hsla.h; s = hsla.s; l = hsla.l; a = hsla.a / 100;
		});
	});

	$effect(() => {
		const newHex = hslaToHex(h, s, l, a);
		untrack(() => { value = newHex; });
	});
	...
	function hslaToHex(h_val: number, s_val: number, l_val: number, a_val: number) {
		h_val /= 360; s_val /= 100; l_val /= 100;
		let r, g, b;
		if (s_val === 0) { r = g = b = l_val; } else {
			...
			r = hue2rgb(h_val + 1/3); g = hue2rgb(h_val); b = hue2rgb(h_val - 1/3);
		}
	...
	>
		<div class="flex items-center justify-between">
			<div class="flex items-center gap-4">
				...
				<div class="flex flex-col">
					<h2 class="font-tiny5 text-3xl text-brand leading-none">Dye Basin</h2>
					<span class="font-serif text-[10px] font-bold uppercase tracking-[0.2em] opacity-30 mt-1">Mix your threads</span>
				</div>
			</div>
			<div class="h-12 w-12 rounded-2xl border-2 border-white shadow-inner" style="background-color: {value};"></div>
		</div>
		...
		<div class="flex flex-col gap-4">
			<div class="flex items-center gap-4 rounded-2xl border border-black/5 bg-white/50 p-4">
				...
				<input type="text" bind:value class="w-full bg-transparent font-mono text-lg font-bold uppercase focus:outline-none" />
			</div>
