<script lang="ts">
	...
	function handleOutsideClick(e: MouseEvent) {
		if ((e.target as HTMLElement).id === 'picker-overlay') onClose();
	}
...
>
	...
	>
		<div class="flex items-center justify-between">
			...
			<button onclick={onClose} class="text-[10px] font-bold tracking-widest uppercase opacity-30 hover:opacity-100">Cancel</button>
		</div>

		<div class="relative flex h-32 items-center justify-center overflow-hidden rounded-3xl border-4 border-white bg-[#eee8d5] shadow-inner">
			<div class="tech-checker absolute inset-0 opacity-30"></div>
			<div class="z-10 h-20 w-20 rounded-full border-4 border-white shadow-lg" style="background-color: {value};"></div>
		</div>

		<div class="flex flex-col gap-5">
			<div class="flex flex-col gap-2">
				<div class="flex justify-between text-[10px] font-black tracking-wider uppercase opacity-30">
					<span>Hue</span>
		...
		<div class="flex flex-col gap-3">
			<div class="relative">
				...
				/>
				<span class="absolute top-1/2 right-3 -translate-y-1/2 text-[8px] font-bold uppercase opacity-20">Hex</span>
			</div>
